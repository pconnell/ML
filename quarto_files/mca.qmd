...
ag_npc = AgglomerativeClustering(
    distance_threshold=1,
    compute_full_tree=True,
    n_clusters=None,
    metric='cosine',
...
y_pred = ag_pc.fit_predict(X_test_pc)
y_pred_npc = ag_npc.fit_predict(X_test_npc)
```


```{python}
X_test_pc['y_pred_ag_pc'] = y_pred
X_test_pc['labels'] = y_test
X_test_npc['y_pred_ag_npc'] = y_pred_npc
X_test_npc['labels'] = y_test
```

...
If we multiply both @eq-svm-pos-pred and @eq-svm-neg-pred...
...
Thus to maximize ...
$$
max\bigg[\sum_{}^{}\lambda_i -\frac{1}{2}\sum_{}^{} \lambda_i \lambda_j y_i y_j x_i^Tx_j : \lambda\in, \lambda > 0\bigg]
$${#eq-dual-form}
...
Code for this portion can be found [here](../src/SVM_code.ipynb) <!--update-->
...
```{python get-data}
predictions = pd.read_csv('../data/SVM_predictions.csv')
...
```{python rbf-scores}
y_pred_npc,y_pred_pc = default[default.columns[1]],default[default.columns[2]]
y_test = default[default.columns[0]]
...
```{python fig-rbf-cm}
#| label: fig-rbf-cm
#| fig-cap: Confusion Matrices for RBF Kernel, C=1
import matplotlib.pyplot as plt
...
```{python}
y_pred_npc,y_pred_pc = linear[linear.columns[1]],linear[linear.columns[2]]
y_true = linear[linear.columns[0]]
results.loc[len(results)] = {
    'Model':'SVM (Kernel=Linear,C=0.5)',
    'Data':'Without Protected Classes',
    'Accuracy':accuracy_score(y_test,y_pred_npc),
    'Precision':precision_score(y_test,y_pred_npc),
    'Recall':recall_score(y_test,y_pred_npc),
...
```{python fig-linear-cm}
#| label: fig-linear-cm
#| fig-cap: Confusion Matrices for Linear Kernel, C=0.5
fig,axes=plt.subplots(nrows=1,ncols=2)
ConfusionMatrixDisplay(
    confusion_matrix(
        y_pred=y_pred_pc,y_true=y_test
...
axes[0].set_title('With Protected\nClasses')
axes[1].set_title('Without Protected\nClasses')
plt.suptitle("Confusion Matrices - SVM Linear C=0.5")
plt.tight_layout()
plt.show()
```
...
```{python}
y_pred_npc,y_pred_pc = sigmoid[sigmoid.columns[1]],sigmoid[sigmoid.columns[2]]
y_true = sigmoid[sigmoid.columns[0]]
results.loc[len(results)] = {
    'Model':'SVM (Kernel=Sigmoid,C=1.5)',
    'Data':'Without Protected Classes',
    'Accuracy':accuracy_score(y_test,y_pred_npc),
    'Precision':precision_score(y_test,y_pred_npc),
    'Recall':recall_score(y_test,y_pred_npc),
...
```{python fig-sig-cm}
#| label: fig-sig-cm
#| fig-cap: Confusion Matrices for Sigmoid Kernel, C=1.5
import matplotlib.pyplot as plt
fig,axes=plt.subplots(nrows=1,ncols=2)
ConfusionMatrixDisplay(
    confusion_matrix(
...
axes[0].set_title('With Protected\nClasses')
axes[1].set_title('Without Protected\nClasses')
plt.suptitle("Confusion Matrices - SVM Sigmoid C=1.5")
plt.tight_layout()
plt.show()
```
...
In this case, leveraging the Receiver Operator Curve - Area Under the Curve (ROC-AUC) may be a more useful metric for comparing the two classes (explain ROC AUC HERE).  With this metric, the base RBF kernel had the best performance, favoring the exclusion of protected class information with a difference of about 0.6%.  For Linear kernel with C=0.5, the performance was similar, but the RBF kernel had 0.1% higher performance on the best performing model.
...
* Effective modeling can be achieved when excluding protected class information in the models
